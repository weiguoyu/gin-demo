...
func (db *Database) New() *Conn {
	return &Conn{
		Session: db.Conn.NewSession(&EventReceiver{}),
	}
}

func NewTx(tx *dbr.Tx) *ConnTX {
	return &ConnTX{
		Tx: tx,
	}
}
...
func (conn *Conn) Select(columns ...string) *SelectQuery {
	return &SelectQuery{conn.Session.Select(columns...), conn.ctx, 0, false, "", ""}
}

func (conn *Conn) SelectBySql(query string, value ...interface{}) *SelectQuery {
	return &SelectQuery{conn.Session.SelectBySql(query, value...), conn.ctx, 0, true, "", ""}
}

func (conn *Conn) SelectAll(columns ...string) *SelectQuery {
	return &SelectQuery{conn.Session.Select("*"), conn.ctx, 0, false, "", ""}
}
...
func (b *SelectQuery) LeftJoinAs(table string, alias string, on interface{}) *SelectQuery {
	b.SelectBuilder.LeftJoin(dbr.I(table).As(alias), on)
	return b
}

func (b *SelectQuery) RightJoinAs(table string, alias string, on interface{}) *SelectQuery {
	b.SelectBuilder.RightJoin(dbr.I(table).As(alias), on)
	return b
}

func (b *SelectQuery) From(table interface{}) *SelectQuery {
...
func (b *SelectQuery) NotDataFilter() *SelectQuery {
	b.hadUsedSelectFilter = true
	return b
}

func (b *SelectQuery) FromAs(table, alias string) *SelectQuery {
	b.tableName = table
	b.tableAlias = alias
	b.SelectBuilder.From(dbr.I(table).As(alias))
	return b
}
...
func (b *SelectQuery) Load(value interface{}) (int, error) {
	b.SelectBuilder.WhereCond = b.selectFilter(b.ctx, b.SelectBuilder.WhereCond)
...
func (b *SelectQuery) CountAndLoad(value interface{}) (count uint32, err error) {
	// Load
	b.SelectBuilder.WhereCond = b.selectFilter(b.ctx, b.SelectBuilder.WhereCond)
	_, err = b.SelectBuilder.Load(value)
	if err != nil {
		return
	}
	...
	b.Order = []dbr.Builder{}
	if b.IsDistinct {
		b.Column = []interface{}{fmt.Sprintf("COUNT(DISTINCT %s)", getColumns(b.Column))}
		b.IsDistinct = false
	} else {
...
func (b *InsertQuery) Record(structValue interface{}) *InsertQuery {
	if len(b.Column) == 0 {
		b.Columns(structutil.GetColumnsFromStruct(structValue)...)
	}
	b.InsertBuilder.Record(structValue)
	return b
}

func (b *InsertQuery) Records(records interface{}) *InsertQuery {
	structValues, ok := sliceutil.TransferInterfaceToSlice(records)
	if !ok {
		return b
	}
	if structValues == nil || len(structValues) == 0 {
		return b
	}
	if len(b.Column) == 0 {
		b.Columns(structutil.GetColumnsFromStruct(structValues[0])...)
	}
...
func (conn *Conn) DeleteFrom(table string) *DeleteQuery {
	return &DeleteQuery{conn.Session.DeleteFrom(table), conn.ctx, conn.DeleteHook, table, false}
}
...
func (b *DeleteQuery) Exec() (sql.Result, error) {
	b.DeleteBuilder.WhereCond = b.deleteFilter(b.ctx, b.DeleteBuilder.WhereCond)
...
func (b *DeleteQuery) MarkUsedDeleteFilter(mark bool) *DeleteQuery {
	b.hadUsedDeleteFilter = mark
	return b
}
...
func (conn *Conn) Update(table string) *UpdateQuery {
	return &UpdateQuery{conn.Session.Update(table), conn.ctx, conn.UpdateHook, table, false}
}

func (b *UpdateQuery) Exec() (sql.Result, error) {
	b.UpdateBuilder.WhereCond = b.updateFilter(b.ctx, b.UpdateBuilder.WhereCond)
...
func (b *UpdateQuery) MarkUsedUpdateFilter(mark bool) *UpdateQuery {
	b.hadUsedUpdateFilter = mark
	return b
}
